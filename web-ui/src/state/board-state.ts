...
function normalizeImages(rawImages: unknown): TaskImage[] | undefined {
	...
	for (const item of rawImages) {
		...
		const img = item as { id?: unknown; data?: unknown; mimeType?: unknown; name?: unknown };
		if (typeof img.id !== "string" || typeof img.data !== "string" || typeof img.mimeType !== "string") {
			continue;
		}
		images.push({
			id: img.id,
